...
COV = --cov=babysit_prs --cov=constants --cov=ghapi --cov=checks --cov=activity \
      --cov=classify --cov=effects --cov=runner
...
lint:
	...
	python3 -m mypy --config-file=.github/linters/.mypy.ini babysit_prs.py constants.py ghapi.py checks.py activity.py classify.py effects.py runner.py
