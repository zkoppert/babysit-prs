...
strict and the PR is cleanly behind, deploys an approved and green authored PR
once per head commit when its repository is enabled with `--review-lab-repo`
or `--preview-repo`,
and sends one macOS notification per PR
...
Guardrails: auto-actions apply only to PRs the user authored; PRs they are
merely assigned to are alert-only. Only required checks are re-run, a branch is
only updated when the base is strict and cleanly behind, and review environment
deploys require explicit repository opt-in plus approval and green known
required checks. Anything ambiguous is left for the user.
...
# Deploy approved, green PRs in selected repositories.
python3 babysit_prs.py --review-lab-repo example-org/backend --preview-repo example-org/frontend
```
...
- Do not deploy a review environment unless the repository was explicitly
  enabled, the PR is approved and non-draft, and every known required check is
  green. Record the deployed head so the same commit is never submitted twice.
